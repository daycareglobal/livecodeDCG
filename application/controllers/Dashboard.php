...
class Dashboard extends CI_Controller {
	...
	function __construct() {
		...
		$this->authKey = "293790ATaXFu8F5d7a6109";
        $this->senderID = "daycare";

		// $this->common_model->checkChildcarePlanPurchase();
	}
 
	function sendOtp() {
		sms_code_send( '9358704504', 'test message' );
	}
	...
	public function index() {
		...
		$business_list = $this->home->getUserBusinessServices($this->user_id);
	...
	function delete_record()
	{
		...
		$this->home->deleteBusinessService($id);
		$data['success'] = true;
...
*/
	...
	function update_profile()
	{
		...
		if (isset($_POST) && !empty($_POST)) {
			...
			$this->form_validation->set_rules('contact_number', 'Contact Number', 'trim|required|min_length[10]|max_length[10]|callback_check_number_exist['.$this->user_id.']');
			...
			$this->form_validation->set_rules('password', 'Password', 'trim|min_length[6]|max_length[15]');
			$this->form_validation->set_rules('confirm_password', 'Confirmation Password', 'trim|matches[password]|min_length[6]|max_length[15]');
			...
			if ($this->form_validation->run()) {
				...
				if ($update_respo) {
					...
					$success = true;
					$data['redirectURL'] = base_url('my-profile');
				} else {
					$success = false;
					$message = 'Technical error, Please try again.';
				}
	    ...
	    $output['user_detail'] = $user_record;
	    $this->load->view('front/includes/header', $output);
		$this->load->view('front/dashboard/update_profile');
		$this->load->view('front/includes/footer');
	}
					...
					if ($business_service_id) {
						...
						if ($_FILES && isset($_FILES['files'])) {
						    ...
						    if ($uploadImgData && !empty($uploadImgData)) {
						    	$this->home->insertBusinessServiceImages($uploadImgData);
						    }
						}
	...
	{		
		...
		$output['business_types'] = $this->business_type->getActiveRecords();
		$output['service_types'] = $this->service_category->getActiveRecords();
		$service_detail = $this->home->getServiceById($id);
		$output['service_detail'] = $service_detail;
		$business_service_id = $id;

		if ($service_detail) {
			$output['service_image'] = $this->home->getBusinessServiceImages($id);
		}



		$seleted_service_types = $this->home->getSelectedBusinessServiceTypes($business_service_id);
		$output['seleted_service_types'] = $seleted_service_types;
		$business_service_days = $this->home->getUserBusinessServiceDays($business_service_id);
		...
		$output['business_service_days'] = $business_service_days;


		/*pr($id);
		pr($output['service_types']);
		pr($seleted_service_types);die;
*/
		...
		if (isset($_POST) && !empty($_POST)) {
			...
			if ($this->form_validation->run()) {
				...
				if (!$failure) {
					$input['user_id'] = $this->user_id;
					//pr($this->input->post('business_type_id'));die;
					$input['business_category_id'] = $this->input->post('business_type_id');
					$input['daycare_name'] = $this->input->post('daycare_name');
					$input['daycare_address'] = $this->input->post('address');
					$input['latitude'] = $this->input->post('latitude');
					...
					//($input);die;
					$res = $this->home->setServiceById($input, $id);
					// $business_service_id = true;

					if ($res) {
						$image_ids = array();
						$imageArrIds = explode(',', $this->input->post('imageArrIds'));
						//pr($imageArrIds);die;

						if ($imageArrIds) {
							$this->home->deleteBusinessServiceImages($imageArrIds);
						}
						$service_images = $this->home->getBusinessServiceImages($business_service_id);
						...
						$service_days = array();
						$this->home->deleteWorkingDays($business_service_id);
						if (isset($_POST['monday']['open'])) {
							$service_days[0]['user_service_id'] = $business_service_id;
							$service_days[0]['day'] = 'monday';
							$service_days[0]['open_time'] = $_POST['monday']['open'];
						...
						$this->home->deleteBusinessServiceType($business_service_id);
						$service_types = $this->input->post('service_types');
		...
		$this->load->view('front/includes/header', $output);
		$this->load->view('front/dashboard/update_service');
		$this->load->view('front/includes/footer');
	}
    ...
    {
        if($this->users->doesNumberExist( $value, $id )) {
