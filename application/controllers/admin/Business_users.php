...
class Business_users extends CI_Controller {
	...
	function index()
	{   
		$output['page_title'] = 'Business User Management';
		$output['left_menu'] = 'user_module';
		$output['left_submenu'] = 'business_users';
		$business_users = $this->user->getRecords('Business');

		foreach ($business_users as $key => $value) {
        	$value->add_date = $this->common_model->getGMTDateToLocalDate($value->add_date);
		}
        
        $output['users'] = $business_users;
		$this->load->view('admin/includes/header',$output);
		$this->load->view('admin/business_users/index');
	...
	function add()
	{
		...
		$output['all_business_type'] = $this->business_types->getActiveRecords();
		$failure = false;
        $message = '';
					...
					if ($user_id) {
						...
						if (!empty($business_type_ids)) {
							...
							if (!empty($business_type)) {
								$this->user->insert_business_types($business_type);
							}
						}
						$message = 'Record added successfully.';
						$success = true;
						$output['redirectURL'] = site_url('admin/business_users');
					}
		...
		$this->load->view('admin/includes/header',$output);
		$this->load->view('admin/business_users/form');
		$this->load->view('admin/includes/footer');
	}
	...
	function update($id)
	{
		...
		$output['status'] = '';
		$output['id'] = $record->id;
		$output['business_name'] = $record->name;
		...
		$output['all_business_type'] = $this->business_types->getActiveRecords();
		$output['selected_business_type'] = $this->user->getSelectedBusinessTypes($record->id);
		$failure = false;
        $message = '';
        ...
        if (isset($_POST) && !empty($_POST)) {
			...
			if ($this->form_validation->run()) {
                ...
                if (!$failure) {
					...
					$user_id = $this->user->update_record($input, $record->id);
					
					if ($user_id) {
						$this->user->delete_business_types($record->id);
						$business_type_ids = $_POST['business_type_id'];
						// $business_type_ids = $this->input->post('business_type_id');
						$business_type = array();
						...
						if (!empty($business_type_ids)) {
							...
							foreach ($business_type_ids as $key => $value) {
								$business_type[$key]['user_id'] = $record->id;
								$business_type[$key]['business_type_id'] = $value;
								$business_type[$key]['add_date'] = $this->common_model->getDefaultToGMTDate(time());
							}
	...
	function rate_user($id)
	{
		...
		$output['left_menu'] = 'user_module';
		$output['left_submenu'] = 'business_users';
		$output['task'] = 'Rate User';
		$output['message'] = '';
		$output['status'] = '';
		$output['id'] = $record->id;
		$output['rating'] = $record->rating;
        $message = '';
        ...
        if (isset($_POST) && !empty($_POST)) {
			...
			if ($this->form_validation->run()) {
				$input = array();
				$input['rating'] = $this->input->post('rating');
				$input['update_date'] = $this->common_model->getDefaultToGMTDate(time());
				$response = $this->user->update_record($input, $record->id);
		...
		$this->load->view('admin/includes/header',$output);
		$this->load->view('admin/business_users/rating');
		$this->load->view('admin/includes/footer');
	}
